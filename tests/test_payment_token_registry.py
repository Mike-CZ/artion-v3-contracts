...
def test_add_when_not_owner(payment_token_registry, user):
    with reverts("Ownable: caller is not the owner"):
        payment_token_registry.add(TOMB_TOKEN, {"from": user})


@given(token_address=strategy('address'))
def test_add_token(payment_token_registry, token_address, owner):
...
def test_add_already_enabled_token(payment_token_registry, owner):
    with reverts("PaymentTokenRegistry: payment token already added"):
        payment_token_registry.add(TOMB_TOKEN, {"from": owner})


def test_remove_when_not_owner(payment_token_registry, user):
    with reverts("Ownable: caller is not the owner"):
        payment_token_registry.remove(TOMB_TOKEN, {"from": user})


def test_remove_token(payment_token_registry, owner):
...
@given(token_address=strategy('address'))
def test_remove_non_existent_token(payment_token_registry, token_address, owner):
    with reverts("PaymentTokenRegistry: payment token does not exist"):
        payment_token_registry.remove(token_address, {"from": owner})


def test_remove_already_removed_token(payment_token_registry, owner):
